...
const CardTour = ({ tour }) => {
  return (
    <div className="max-w-sm h-[454px] rounded-xl overflow-hidden shadow-lg bg-gradient-to-r from-green-300 to-teal-400 transform transition-all duration-300 hover:shadow-xl">
      {/* Hình ảnh Tour */}
      <div className="relative h-1/3 overflow-hidden rounded-t-xl">
        <Image
          src={tour.image}
          alt={tour.name}
          layout="fill"
      ...
      <div className="flex flex-col justify-between h-full p-6 bg-white/80 backdrop-blur-sm rounded-b-xl">
        {/* Địa điểm */}
